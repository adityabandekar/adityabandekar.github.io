(function() {
    ...
    const colorScale = d3.scaleThreshold()
        .domain([1, 10, 50, 100, 500, 1000, 5000])
        .range(d3.schemeYlOrRd[8]);

    Promise.all([
        d3.json("https://raw.githubusercontent.com/holtzy/D3-graph-gallery/master/DATA/world.geojson"),
        d3.csv("co2_map_data.csv")
    ]).then(function([world, data]) {
        let currentYear = "2022";

        function getCO2(iso_code, year) {
        ...
        function drawSparkline(selection, countryIso) {
            ...
            const y = d3.scaleLinear()
                .domain([0, d3.max(countryHistory, d => +d.co2)])
                .range([sh, 2]);
        ...
        mapGroup.selectAll("path")
            ...
            .attr("d", path)
            .style("fill", d => {
                const val = getCO2(d.id, currentYear);
            ...
            .on("mouseover", function(event, d) {
                ...
                tooltip.append("div")
                    ...
                    .text(`${val ? val.toLocaleString() : 0}M tonnes (${currentYear})`);
